    ...
    return (
      ...
      >
        <div className="group relative cursor-pointer" onClick={openModal}>
          <button className="absolute left-1/2 top-1/2 flex w-fit -translate-x-1/2 -translate-y-1/2 items-center gap-2 rounded-lg bg-sky-500 px-6 py-3 text-lg font-bold transition-colors group-hover:bg-gray-50 group-hover:text-sky-500">
            Desbloquear
          ...
          <div className="select-none blur-md">
            <section className="border-b border-gray-700 p-4">
            ...
            <section className="flex flex-col gap-1 border-b border-gray-700 p-4 text-center">
              <p>Pontuação Geral</p>
  ...
  return (
    <AssetDataContainer title="RANKING WIND" description={windScoreDescription}>
      ...
      <section className="grid grid-cols-1 place-items-center gap-4 p-4 sm:grid-cols-2 md:grid-cols-4">
        {scoreData.map(({ title, score }) => (
          <div key={title} className="flex flex-col items-center">
            <p className="text-lg font-semibold text-green-500">{title}</p>
            <data value={score === null ? 'N/A' : formatToRatio(score)}>
              {score === null ? 'N/A' : formatToRatio(score)}
            </data>
          </div>
